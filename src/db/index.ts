...
export const productService = {
  // 获取所有商品
  getAll: async (page = 1, pageSize = 10, keyword = ''): Promise<{ list: Product[], total: number }> => {
    try {
      // 获取所有商品数据
  ...
  // 更新商品
  update: async (id: string, productData: Partial<Omit<Product, 'id' | 'createdAt' | 'updatedAt'>>): Promise<Product> => {
    try {
      const existingProduct = await productsStore.getItem(id) as Product | null;
      if (!existingProduct) {
...
export const orderService = {
  // 创建订单
  create: async (orderData: CreateOrderData): Promise<{ orderId: string; createdAt: Date }> => {
    try {
      const now = new Date();
  ...
  // 获取销售统计
  getSalesStatistics: async (startDate: string, endDate: string, interval: 'day' | 'week' | 'month' = 'day'): Promise<SalesStatistics[]> => {
    try {
      // 获取所有订单
      let allOrders: Order[] = [];
      ...
      // 转换为数组并排序
      const statistics = Array.from(statisticsMap.values()).sort((a, b) => 
  ...
  // 获取热销商品排行
  getTopProducts: async (limit = 10, startDate?: string, endDate?: string): Promise<TopProduct[]> => {
    try {
      // 实现热销商品排行逻辑
      ...
      // 统计每个商品的销售数量和金额
      const relevantItems = filteredOrderIds.length > 0 
        ? allOrderItems.filter(item => filteredOrderIds.includes(item.orderId))
        : allOrderItems;
  ...
  // 获取订单详情
    getOrderDetails: async (orderId: string): Promise<{ order: Order | null; items: OrderItem[] }> => {
      try {
    ...
    update: async (order: Order, items: OrderItem[]): Promise<void> => {
      try {
        ...
        for (const item of items) {
          await orderItemsStore.setItem(item.id, { ...item, orderId: order.id });
        }
      } catch (error) {
